 ...
 * `review-check 20` says so, on every pair at once and not by a little. Comparing the
 * readings over the seeds both tiers won, so stalls can't be blamed:
 *
 ...
 *
 * Colonel stops separating from `easy`, the top pair changes sign, the error bars
 * widen everywhere, the check fails outright on the unrestricted reading, and it
 * costs four and a half times as much.
 *
 * The reason is worth keeping, because it says where this technique belongs at all:
 * **roll out where one ply is blind, never where it is exact.** An attack's value is
 * already integrated over its exact outcome distribution, so a rollout replaces an
 * exact number with that same number plus a policy's guess about the rest of the
 * turn — strictly more noise, no more signal. A deploy has no exact number to
 * replace: armies in hand buy nothing until they're spent, which is what makes the
 * turn context worth its cost there and nowhere else.
 */
